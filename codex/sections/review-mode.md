...
cd "$_REPO_ROOT"
# The 1200s wrapper sits BELOW the 1260s Bash gate so the wrapper fires FIRST
...
_gstack_codex_timeout_wrapper 1200 codex review --base <base> -c 'sandbox_mode="read-only"' -c 'model="gpt-6-astra"' -c 'review_model="gpt-6-astra"' -c 'model_reasoning_effort="max"' -c 'web_search="cached"' < /dev/null 2>"$TMPERR"
_CODEX_EXIT=$?
...
_gstack_codex_timeout_wrapper 1200 codex exec -s read-only "$(cat "$_PROMPT_FILE")" -c 'model="gpt-6-astra"' -c 'review_model="gpt-6-astra"' -c 'model_reasoning_effort="max"' -c 'web_search="cached"' < /dev/null 2>"$TMPERR"
_CODEX_EXIT=$?
rm -f "$_PROMPT_FILE"
if [ "$_CODEX_EXIT" = "124" ]; then
  _gstack_codex_log_event "codex_timeout" "1200"
...
Use `timeout: 1260000` on the Bash call for either path. The Bash gate sits ABOVE the
1200s wrapper deliberately: the wrapper fires first with its explicit exit-124 message,
