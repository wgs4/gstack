...
import { codexPreflight, codexErrorHandling, CODEX_WEB_SEARCH_FLAG, CODEX_REVIEW_MODEL_FLAGS, CODEX_REVIEWER_MODEL, CODEX_REVIEWER_EFFORT, codexReviewModelAssert, CC_BACKGROUND_DEFAULT_SINCE } from './constants';
...
codex exec "$(cat "$CODEX_PROMPT_FILE")" -C "$_REPO_ROOT" -s read-only ${CODEX_REVIEW_MODEL_FLAGS} ${CODEX_WEB_SEARCH_FLAG} < /dev/null 2>"$TMPERR_OH"
\`\`\`
...
_gstack_codex_timeout_wrapper 1200 codex exec "${CODEX_BOUNDARY}Review the changes on this branch against the base branch. Run DIFF_BASE=$(git merge-base origin/<base> HEAD) && git diff "$DIFF_BASE" to see the diff. Your job is to find ways this code will fail in production. Think like an attacker and a chaos engineer. Find edge cases, race conditions, security holes, resource leaks, failure modes, and silent data corruption paths. Be adversarial. Be thorough. No compliments — just the problems. End your output with ONE line in the canonical format \`Recommendation: <action> because <one-line reason naming the most exploitable finding>\`. Generic reasons like 'because it's safer' do not qualify; the reason must point to a specific finding or no-fix rationale." -C "$_REPO_ROOT" -s read-only ${CODEX_REVIEW_MODEL_FLAGS} ${CODEX_WEB_SEARCH_FLAG} < /dev/null 2>"$TMPERR_ADV"
\`\`\`
...
# unwrapped fallback), added in #1056 but never wired into this call site.
source ~/.claude/skills/gstack/bin/gstack-codex-probe 2>/dev/null || true
_gstack_codex_timeout_wrapper 1200 codex review --base <base> ${CODEX_REVIEW_MODEL_FLAGS} ${CODEX_WEB_SEARCH_FLAG} < /dev/null 2>"$TMPERR"
\`\`\`
...
_REPO_ROOT=$(git rev-parse --show-toplevel) || { echo "ERROR: not in a git repo" >&2; exit 1; }
codex exec "<prompt>" -C "$_REPO_ROOT" -s read-only ${CODEX_REVIEW_MODEL_FLAGS} ${CODEX_WEB_SEARCH_FLAG} < /dev/null 2>"$TMPERR_PV"
\`\`\`
...
${codexReviewModelAssert('TMPERR_PV')}
...
_REPO_ROOT=$(git rev-parse --show-toplevel) || { echo "ERROR: not in a git repo" >&2; exit 1; }
codex exec "<prompt>" -C "$_REPO_ROOT" -s read-only ${CODEX_REVIEW_MODEL_FLAGS} ${CODEX_WEB_SEARCH_FLAG} < /dev/null 2>"$TMPERR_DOC"
\`\`\`
